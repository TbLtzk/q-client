...
func New(eth Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(header *types.Header) bool, accountManager *accounts.Manager, gpp core.GasPriceProvider) *Miner {
	miner := &Miner{
		mux:     mux,
		eth:     eth,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, eth, mux, isLocalBlock, true, accountManager, gpp),
	}
	miner.wg.Add(1)
	go miner.update()
	return miner
