...
func (w *worker) resultLoop() {
	...
	for {
		...
		case block := <-w.resultCh:
			...
			if err != nil {
				log.ErrorAndNotify("Failed writing block to chain", "err", err)
				continue
			}
...
func (w *worker) applyTransaction(env *environment, tx *types.Transaction) (*types.Receipt, error) {
	...
	// Always execute transactions against the block header coinbase so producer-side
	// fee accounting matches import-time validation context.
