	...
	for hash := range BadHashes {
		if header := bc.GetHeaderByHash(hash); header != nil {
			...
			if headerByNumber != nil && headerByNumber.Hash() == header.Hash() {
				log.ErrorAndNotify("Found bad hash, rewinding chain", "number", header.Number, "hash", header.ParentHash)
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db ethdb.KeyValueWriter, header *types.Header) (uint64, bool) {
		...
		if currentBlock := bc.CurrentBlock(); currentBlock != nil && header.Number.Uint64() <= currentBlock.NumberU64() {
			...
			if newHeadBlock == nil {
				log.ErrorAndNotify("Gap in the chain, rewinding to genesis", "number", header.Number, "hash", header.Hash())
				...
				for {
					...
					if _, err := state.New(newHeadBlock.Root(), bc.stateCache, bc.snaps); err != nil {
						...
						if pivot == nil || newHeadBlock.NumberU64() > *pivot {
							...
							log.ErrorAndNotify("Missing block in the middle, aiming genesis", "number", newHeadBlock.NumberU64()-1, "hash", newHeadBlock.ParentHash())
							newHeadBlock = bc.genesisBlock
						} else {
		...
		for _, offset := range []uint64{0, 1, TriesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if err := triedb.Commit(recent.Root(), true, nil); err != nil {
					log.ErrorAndNotify("Failed to commit recent state trie", "err", err)
				}
			}
		}
...
func (bc *BlockChain) SwitchToSidechain(chain []*types.Block, failedBlock *types.Block) (err error) {
	...
	// Otherwise, try to reorg the chain
	...
	if ancestorHeader := rawdb.FindCommonAncestor(bc.db, bc.CurrentHeader(), chain[0].Header()); ancestorHeader != nil {
		...
		for {
			...
			if prevBlock == nil {
				...
				return
				// return i, fmt.Errorf("failed to find parent block")
			}
		}
		log.Info("====================================")
		...
		_, _, err = bc.RevalidateChain(prevBlock.Number().Uint64()+1, chain, canonical)
		if err != nil {
