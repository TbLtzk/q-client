...
func TestInvalidTransactions(t *testing.T) {
	...
	pool.priceLimit = uint256.NewInt(1000)
...
func TestReplacement(t *testing.T) {
	...
	// have this: threshold-2*threshold/100 instead of just threshold - 1 in order to test 1% underpay tolerance
	if err := pool.addRemote(pricedTransaction(0, 100001, big.NewInt(threshold-2*threshold/100), key)); err != txpool.ErrReplaceUnderpriced {
	...
	if err := pool.addRemote(pricedTransaction(2, 100001, big.NewInt(threshold-2*threshold/100), key)); err != txpool.ErrReplaceUnderpriced {
