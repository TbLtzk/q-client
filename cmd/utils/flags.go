...
import (
	...
	"gitlab.com/q-dev/go-ethereum/ethstats"
	"gitlab.com/q-dev/go-ethereum/graphql"
	"gitlab.com/q-dev/go-ethereum/les"
	"gitlab.com/q-dev/go-ethereum/log"
	"gitlab.com/q-dev/go-ethereum/metrics"
	"gitlab.com/q-dev/go-ethereum/metrics/influxdb"
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	config, hash, err := core.SetupGenesisBlock(chainDb, MakeGenesis(ctx))
	if err != nil {
		Fatalf("%v", err)
	}
	var engine consensus.Engine
	if config.Clique != nil {
		engine = clique.New(config.Clique, chainDb, hash)
	} else {
