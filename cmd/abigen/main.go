...
var (
	...
	truffleFlag = cli.StringFlag{
		Name:  "truffle-json",
		Usage: "Path to json file generated by truffle compile",
	}
...
func abigen(c *cli.Context) error {
	utils.CheckExclusive(c, abiFlag, jsonFlag, truffleFlag, solFlag, vyFlag) // Only one source can be selected.
	...
	} else if c.GlobalString(truffleFlag.Name) != "" {
		jsonOutput, err := ioutil.ReadFile(c.GlobalString(truffleFlag.Name))
		if err != nil {
			utils.Fatalf("Failed to read json file from truffle: %v", err)
		}

		abiStr, binStr, err := compiler.ParseTruffleJSON(jsonOutput)
		if err != nil {
			utils.Fatalf("Failed to read contract information from truffle json output: %v", err)
		}

		abis = append(abis, *abiStr)
	...
	code, err := bind.Bind(types, abis, bins, sigs, c.String(pkgFlag.Name), lang, libs, aliases, structExc)
	if err != nil {
